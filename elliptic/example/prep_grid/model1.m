...
for i=p2+1:p3
    bz2(i,1) = origin_x + bz2(p2,1) + cos(0.5*pi*70/90)*(i-p2)*dh;
    bz2(i,2) = origin_z + bz2(p2,2) - sin(0.5*pi*70/90)*(i-p2)*dh;
end
for i=p3+1:p4
...
for i=p4+1:p5
    bz2(i,1) = origin_x + bz2(p4,1) + cos(0.5*pi*70/90)*(i-p4)*dh;
    bz2(i,2) = origin_z + bz2(p4,2) + sin(0.5*pi*70/90)*(i-p4)*dh;
end
for i=p5+1:p6
