...
int main(int argc, char** argv)
{
  ...
  if(par->flag_sample == 1)
  {
    ...
    grid_sample(gdcurv_new,gdcurv,par->sample_factor_xi,par->sample_factor_zt);
    fprintf(stdout,"***********************************\n");
    fprintf(stdout,"***********************************\n");
    fprintf(stdout,"***********************************\n");
    ...
    int indx_i, indx_k;
    float dL_min;
    cal_min_dist(gdcurv_new, &indx_i, &indx_k, &dL_min);
    fprintf(stdout,"indx is (%d,%d),dL_min_global is %f\n",
            indx_i, indx_k, dL_min);
    fflush(stdout);
  } else {
    ...
    int indx_i, indx_k;
    float dL_min;
    cal_min_dist(gdcurv, &indx_i, &indx_k, &dL_min);
    fprintf(stdout,"indx is (%d,%d),dL_min_global is %f\n",
            indx_i, indx_k, dL_min);
    fflush(stdout);
  }
